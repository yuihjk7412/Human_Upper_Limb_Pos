	...
	//printf("packet decode!!");
        ...
        case kStatus_LenHigh:
            ...
            status = kStatus_CRCLow;
	     	//printf("data length:%d",RxPkt->payload_len);
            break;
        case kStatus_CRCLow:
            CRCReceived = c;
		...
		//printf("%x",c);
		//printf("data");
            if(RxPkt->type == 0xA7 && RxPkt->ofs >= 8)	//ping response mode
            {
                RxPkt->payload_len = 8;
                EventHandler(RxPkt);
                status = kStatus_Idle;
            }
            if(RxPkt->ofs >= MAX_PACKET_LEN)	//data outflow, error occur
            {
		//printf("outflow");
		//printf("%d",MAX_PACKET_LEN);
		//printf("%d",kItemRotationQuat);
                status = kStatus_Idle;
                return CH_ERR;   
            }
